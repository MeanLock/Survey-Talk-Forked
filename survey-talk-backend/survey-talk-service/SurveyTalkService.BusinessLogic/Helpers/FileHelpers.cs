...
namespace SurveyTalkService.BusinessLogic.Helpers
{
    public class FileHelpers
    {
        ...
        public async Task<string> GetImageBase64(string rootFolderPath, string folderName, string fileName)
        {
            try
            {
                // Xác định đường dẫn thư mục chứa nhóm ảnh
                string folderPath = Path.Combine(rootFolderPath, folderName);
                string fullFileName = await GetFullFileName(rootFolderPath + "/" + folderName, fileName);
                ...
                // Console.WriteLine($"\n\nGetting image base64 from: {filePath}");
