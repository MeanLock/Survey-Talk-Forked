...
namespace SurveyTalkService.BusinessLogic.Services.OpenAIServices._4oMini
{
    public class OpenAI4oMiniService
    {
        ...
        /////////////////////////////////////////////////////////////
        // test
        public async Task<JArray> GetFilterTagSummariesAsync(List<FilterTag> filterTags, List<JObject> surveyResponses)
        {
            // Chuẩn bị function schema cho OpenAI function calling
            var functionSchema = new JObject
            {
                ["name"] = "summarize_filter_tags",
                ["description"] = "Tóm tắt ngắn gọn tiếng Việt cho từng filter tag dựa trên nội dung các response.",
                ["parameters"] = new JObject
                {
                    ["type"] = "object",
                    ["properties"] = new JObject
                    {
                        ["result"] = new JObject
                        {
                            ...
                            ["items"] = new JObject
                            {
                                ...
                                {
                                    ["filterTagId"] = new JObject { ["type"] = "integer" },
                                    ["filterTagName"] = new JObject { ["type"] = "string", ["description"] = "Tên của filter tag." },
                                    ["summary"] = new JObject { ["type"] = "string", ["description"] = "Tóm tắt ngắn gọn tiếng Việt hoặc null nếu không liên quan." }
                                },
                                ["required"] = new JArray { "filterTagId", "filterTagName", "summary" }
                            }
                        }
                    },
                    ["required"] = new JArray { "result" }
            ...
            // Chuẩn bị prompt
            var prompt = $@"Bạn là AI chuyên phân tích survey. Dưới đây là danh sách filter tag và danh sách response của 1 người dùng. Hãy trả về 1 mảng JSON, mỗi phần tử gồm filterTagId, filterTagName và summary tiếng Việt thật ngắn gọn, súc tích, đủ ý (không cần diễn giải chi tiết, không lặp lại nội dung câu hỏi), tóm tắt dựa trên nội dung các response. Nếu ở tag nào đó không được đề cập trong các yếu tố đầu vào (hoặc không chất lọc được thông tin) thì Summary phải để null, bạn buộc phải tuân theo rule này (không được viết các câu kiểu như 'không có thông tin', 'không có dữ liệu', 'không có', 'không rõ', v.v.). Không tự chế thêm tag, chỉ dùng đúng filterTagId đã cho. Kết quả trả về đúng format function calling OpenAI yêu cầu.\n\nfilterTags: {JArray.FromObject(filterTags)}\n\nsurveyResponses: {JArray.FromObject(surveyResponses)}";
            ...
            {
                httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", _openAIConfig.ApiKey);
                // Sử dụng BaseUrl hoặc tên property đúng trong config của bạn
                httpClient.BaseAddress = new Uri(_openAIConfig.BaseUrl);
                var content = new StringContent(body.ToString(), Encoding.UTF8, "application/json");
                var response = await httpClient.PostAsync("/v1/chat/completions", content);
                response.EnsureSuccessStatusCode();
                var responseString = await response.Content.ReadAsStringAsync();
                var responseJson = JObject.Parse(responseString);
                // Lấy function_call arguments
                var functionCall = responseJson["choices"]?[0]?["message"]?["function_call"];
                if (functionCall != null && functionCall["arguments"] != null && !string.IsNullOrEmpty(functionCall["arguments"]?.ToString()))
                {
                    var arguments = functionCall["arguments"].ToString();
                    var resultObj = JObject.Parse(arguments);
                    return resultObj["result"] as JArray ?? new JArray();
                }
                return new JArray();
            }
        }
        ...
        // test
        public async Task<JArray> GetMergedFilterTagSummariesAsync(List<FilterTag> filterTags, List<JArray> surveyTakenResultTagFilterGroups)
        {
            // Chuẩn bị function schema cho OpenAI function calling
            var functionSchema = new JObject
            {
                ["name"] = "merge_filter_tag_summaries",
                ["description"] = "Tóm tắt chung nhất tiếng Việt cho từng filter tag dựa trên nhiều summary từ các group.",
                ["parameters"] = new JObject
                {
                    ["type"] = "object",
                    ["properties"] = new JObject
            ...
            // Chuẩn bị prompt
            var prompt = $@"Bạn là AI chuyên phân tích survey. Dưới đây là danh sách filter tag và các nhóm summary (mỗi nhóm là 1 lần làm survey của 1 người, mỗi phần tử gồm filterTagId, filterTagName, summary). Hãy gộp các summary cùng filterTagId thành 1 summary chung nhất, ngắn gọn, súc tích, đủ ý (không cần diễn giải chi tiết, không lặp lại nội dung câu hỏi). Nếu ở tag nào đó không được đề cập trong các yếu tố đầu vào (hoặc không chất lọc được thông tin) thì Summary phải để null, bạn buộc phải tuân theo rule này (không được viết các câu kiểu như 'không có thông tin', 'không có dữ liệu', 'không có', 'không rõ', v.v.). Kết quả trả về đúng format function calling OpenAI yêu cầu.\n\nfilterTags: {JArray.FromObject(filterTags)}\n\nsurveyTakenResultTagFilterGroups: {JArray.FromObject(surveyTakenResultTagFilterGroups)}";
            ...
            var body = new JObject
            {
                ...
                ["functions"] = new JArray { functionSchema },
                ["function_call"] = new JObject { ["name"] = "merge_filter_tag_summaries" }
            };
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                if (functionCall != null && functionCall["arguments"] != null && !string.IsNullOrEmpty(functionCall["arguments"]?.ToString()))
                {
                    var arguments = functionCall["arguments"]?.ToString() ?? string.Empty;
