...
CREATE TABLE SurveyQuestion (
    ...
    referenceSurveyQuestionId UNIQUEIDENTIFIER NULL REFERENCES SurveyQuestion(id),
    deletedAt DATETIME NULL
);
...
CREATE TABLE SurveyOption (
    id UNIQUEIDENTIFIER PRIMARY KEY DEFAULT NEWID(),
    surveyQuestionId UNIQUEIDENTIFIER NOT NULL REFERENCES SurveyQuestion(id),
    content NVARCHAR(500) NOT NULL,
    [order] TINYINT NOT NULL
);
...
CREATE TABLE SurveyMarketTransaction (
    id INT IDENTITY PRIMARY KEY,
    accountId INT NOT NULL REFERENCES Account(id),
    dataPurchaseId INT NOT NULL REFERENCES DataPurchase(id),
    amount DECIMAL(18,2) NOT NULL,
    profit DECIMAL(18,2) NULL,
    transactionTypeId INT NOT NULL REFERENCES TransactionType(id),
    transactionStatusId INT NOT NULL REFERENCES TransactionStatus(id),
    createdAt DATETIME NOT NULL DEFAULT(CAST(SYSDATETIMEOFFSET() AT TIME ZONE 'N. Central Asia Standard Time' AS DATETIME))
);

-- SurveyCommunityTransaction
CREATE TABLE SurveyCommunityTransaction (
    id INT IDENTITY PRIMARY KEY,
    accountId INT NOT NULL REFERENCES Account(id),
    surveyId INT NOT NULL REFERENCES Survey(id),
    amount DECIMAL(18,2) NOT NULL,
...
ALTER TABLE DataPurchaseDetail
DROP COLUMN purchasedPrice;
