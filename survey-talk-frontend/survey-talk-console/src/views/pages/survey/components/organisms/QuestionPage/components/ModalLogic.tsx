...
import type { OptionType, QuestionType } from "@/core/types/tools";
...
const [localJumpLogics, setLocalJumpLogics] = React.useState<JumpLogicType[]>(
        (configJson?.JumpLogics || []) as JumpLogicType[]
    );
    ...
    const handleDeleteLogic = (logicIndex: number) => {
     const newLogics = localJumpLogics.filter((_, index) => index !== logicIndex);
        setLocalJumpLogics(newLogics);        // handleUpdateQuestion("ConfigJson", {
        //     ...configJson,
   ...
   const handleAddCondition = (logicIndex: number) => {
        const logic = localJumpLogics[logicIndex];
        const newCondition = {
            QuestionId: "",
        ...
        setLocalJumpLogics(newLogics);
    };


      const handleDeleteCondition = (logicIndex: number, conditionIndex: number) => {
        const logic = localJumpLogics[logicIndex];
  ...
  const handleUpdateCondition = (
        ...
        const newLogics = [...localJumpLogics];
        const logic = newLogics[logicIndex];
        ...
        <div style={styles.modalOverlay}>
            <div className="bg-white rounded-lg w-[90%] max-w-3xl max-h-[90vh] flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto p-4">
                    {localJumpLogics.map((logic, logicIndex) => (
                        <div
                            key={logicIndex}
